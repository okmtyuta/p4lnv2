...
class TrainResultLoader:
    ...
    def _set_epoch_result(cls, group: h5py.Group, epoch_result: EpochResult):
        ...
        group.create_dataset("pearsonr", data=epoch_result["pearsonr"])
        group.create_dataset("mean_squared_error", data=epoch_result["mean_squared_error"])
        group.create_dataset("root_mean_squared_error", data=epoch_result["root_mean_squared_error"])
        group.create_dataset("mean_absolute_error", data=epoch_result["mean_absolute_error"])
    ...
    def save_as_h5(self, path: str):
        with h5py.File(path, mode="w") as f:
            ...
            for prop_name in self._train_result["output_props"]:
                ...
                self._set_epoch_result(max_accuracy_group, self._train_result["max_accuracy_result"][prop_name])

                train_group = prop_group.create_group("train")
                train_epochs_group = train_group.create_group("epochs")
                train_epoch_results = self._train_result["train_result"][prop_name]
                ...
                evaluate_epoch_results = self._train_result["evaluate_result"][prop_name]
    ...
    def _load_epoch_result(self, group: h5py.Group):
        ...
        pearsonr = group["pearsonr"][()].item()
        mean_squared_error = group["mean_squared_error"][()].item()
        root_mean_squared_error = group["root_mean_squared_error"][()].item()
        mean_absolute_error = group["mean_absolute_error"][()].item()
    ...
    def from_h5(cls, path: str):
        ...
        with h5py.File(path, mode="r") as f:
            ...
            for prop_name in output_props:
                ...
                max_accuracy_result[prop_name] = cls._load_epoch_result(max_accuracy_group)
        ...
        train_result: TrainResult = {
            ...
            "evaluate_result": _evaluate_result,
